...
class JCR(core.components.Component):
        ...
        try:
            while True:
                ...
                if 'coverage info file' in coverage_info:
                    ...
                    add_to_coverage(total_coverage_infos[job_id][rule_spec], loaded_coverage_info)
                    for file in loaded_coverage_info.values():
                        arcfiles[job_id][rule_spec][file[0]['file name']] = file[0]['arcname']
                    del loaded_coverage_info

                    counters.setdefault(job_id, dict())
                    counters[job_id].setdefault(rule_spec, 0)
                    counters[job_id][rule_spec] += 1
                    if counters[job_id][rule_spec] >= 10:
                        self.__read_data(total_coverage_infos, job_id, rule_spec)
                        self.__save_data(total_coverage_infos, job_id, rule_spec)
                        self.__clean_data(total_coverage_infos, job_id, rule_spec)
                        counters[job_id][rule_spec] = 0
                    ...
                    for rule_spec in counters[job_id]:
                        self.__read_data(total_coverage_infos, job_id, rule_spec)
                        coverage_info = total_coverage_infos[job_id][rule_spec]
                        total_coverage_dir = self.__get_total_cov_dir(job_id, rule_spec)
    ...
    def __get_total_cov_dir(self, job_id, rule):
    ...
    def __read_data(self, cache, job_id, rule):
        file_name = os.path.join(self.__get_total_cov_dir(job_id, rule), self.COVERAGE_FILE_NAME)
        if os.path.isfile(file_name):
            with open(file_name, 'r', encoding='utf8') as fp:
                large_cache = json.load(fp)
        else:
            large_cache = dict()
        cache.setdefault(job_id, dict())
        cache[job_id].setdefault(rule, dict())
        for file_name in large_cache:
            cache[job_id][rule].setdefault(file_name, {
            ...
            for path in ('covered lines', 'covered functions'):
                for line, value in large_cache[file_name][path].items():
                    cache[job_id][rule][file_name][path].setdefault(line, 0)
                    cache[job_id][rule][file_name][path][line] += value

    def __save_data(self, cache, job_id, rule):
        file_name = os.path.join(self.__get_total_cov_dir(job_id, rule), self.COVERAGE_FILE_NAME)
        cache.setdefault(job_id, dict())
        cache[job_id].setdefault(rule, dict())
        with open(file_name, 'w', encoding='utf8') as fp:
            json.dump(cache[job_id][rule], fp)
        ...
        if self.completeness in ('partial', 'lightweight'):
            with open(self.coverage_file, encoding='utf-8') as fp:
                ...
                for path, files in all_files.items():
                    ...
                    if self.completeness == 'lightweight' and not path.startswith(self.shadow_src_dir):
