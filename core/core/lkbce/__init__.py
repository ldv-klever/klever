...
class LKBCE(core.components.Component):
    ...
    def build_linux_kernel(self):
        ...
        if 'modules dep file' in self.conf['Linux kernel']:
            # Should parse user file of module dependencies.
            # This file has more priority, that generated by depmod after modules_install command.
            self.parse_linux_kernel_mod_deps(self.conf['Linux kernel']['modules dep file'])
            self.mqs['Linux kernel module deps'].put(self.conf['Linux kernel']['module deps'])
        ...
        if not self.conf['Linux kernel'].get('build kernel', False) and 'modules' in self.conf['Linux kernel']:
