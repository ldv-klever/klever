...
class VertexMerge(AbstractDivider):

    def __init__(self, logger, conf):
        super(VertexMerge, self).__init__(logger, conf)
        self._max_iters = self.conf['VOG divider'].get('max iters', 200)
        self._max_locs = self.conf['VOG divider'].get('max locs')
        self._max_files = self.conf['VOG divider'].get('max files')
        self._only_same_subsystems = self.conf['VOG divider'].get('same subsystems')
        self._file_sizes = {}

    def divide(self):
        dependencies, root_files = self._build_dependencies()
        self._graph = self._convert_graph(dependencies, root_files)
        ...
        self.logger.debug("Iterations {0}".format(i))
        modules = {}
        for node in sorted(self._graph.values()):
            module_desc_files = set([self._cmd_graph_ccs[cur]['id'] for cur in node.elems])
            module_in_files = set()
            for elem in node.elems:
                module_in_files.update(self._cmd_graph_ccs[elem]['in'])
            self.logger.debug('Create module with {0} in files'.format(list(module_in_files)))
            modules.update(self._create_module_by_desc(module_desc_files, module_in_files))
        return modules

    def _convert_graph(self, dependencies, root_files):
        ret = {}
        processed = set()
        process = root_files
        while process:
            ...
            if c not in self._cmd_graph_ccs:
                continue

            if c not in ret:
                ret[c] = Node(c, self._get_locs(c))

            # todo: another one useless sorting
            for child in sorted(dependencies.get(c, [])):
