...
class VOG(core.components.Component):
    ...
    def generate_verification_objects(self):
        ...
        # divider = divider(self.logger, self.conf)
        if not self.conf['project'].get('is base cached'):
            ...
            with self.locks['build']:
                self.prepare_and_build(adapter)

        self.common_prj_attrs = adapter.attributes # + strategy.attributes + divider.attributes
