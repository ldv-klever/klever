...
from core.vtg.emg.generators.linuxModule.process import ExtendedAccess, Call
        ...
        if uncalled_callbacks:
            ...
            if uncalled_callbacks and not conf.get('ignore missed callbacks', True):
                ...
                logger.warning("There are callbacks from category {!r} which are not called at all in the "
                               ...
                               "configuration properties if you would like to terminate.".format(category, names))
...
def __import_kernel_models(logger, conf, interfaces, collection, chosen):
    for func, process in ((f, process) for f, process in collection.models.items()
                          if f in (i.name for i in interfaces.function_interfaces)):
        ...
        for label in new_process.labels.keys():
            ...
            if new_process.labels[label].interfaces and len(new_process.labels[label].interfaces) > 0:
                for interface in (i for i in new_process.labels[label].interfaces
                    ...
                    new_process.labels[label].assign_label_interface(interfaces.get_intf(interface))


def __choose_processes(logger, conf, interfaces, category, chosen, collection):
    estimations = {}
    for process in collection.environment.values():
        logger.debug("Estimate correspondence between process {!r} and category {!r}".format(process.name, category))
        estimations[process.name] = __match_labels(logger, interfaces, process, category)
        ...
        if peers and len(peered_processes) == 0:
            ...
            if uncalled_callbacks and len(callback_labels) > 0:
                ...
                for intf in uncalled_callbacks:
                    callback_labels[-1].assign_label_interface(intf)
...
def __match_labels(logger, interfaces, process, category):
        ...
        for action in process.calls:
            ...
            if len(label.interfaces) > 0 and label.name not in label_map["matched labels"]:
                ...
                for container in interfaces.containers(category):
                    intfs = __resolve_interface(logger, interfaces, container, tail)
                    if intfs:
                        __add_label_match(logger, interfaces, label_map, label, container.identifier)
            ...
            for func in functions:
                ...
                for par in action.parameters:
                    ...
                    if p_tail:
                        for container in interfaces.containers(category):
                            intfs = __resolve_interface(logger, interfaces, container, p_tail)
                            if intfs:
                                __add_label_match(logger, interfaces, label_map, p_label, container.identifier)
                ...
                for pr, par in enumerate(f_intfs):
                    ...
                    if len(matched) == 0 and par.identifier not in pre_matched:
                        ...
                        if len(unmatched) > 0:
                            __add_label_match(logger, interfaces, label_map, unmatched[0], par.identifier)
                        else:
                            rsrs = [label[0] for label in labels if label[0].resource]
                            if len(rsrs) > 0:
                                __add_label_match(logger, interfaces, label_map, rsrs[0], par.identifier)
                ...
                for container in matched_containers:
                    for container_intf in [interfaces.get_intf(intf) for intf in
                        ...
                        for f_intf in [intf for intf in container_intf.field_interfaces.values()
                            ...
                            __add_label_match(logger, interfaces, label_map, callback, f_intf.identifier)
        if len(unmatched_callbacks) > 0:
            for callback in unmatched_callbacks:
                for intf in [intf for intf in interfaces.callbacks(category)
                    ...
                    __add_label_match(logger, interfaces, label_map, callback, intf.identifier)
    ...
    for label in [l for l in label_map["unmatched labels"] if process.labels[l].container]:
        ...
        if "%{}%".format(label) not in acceses and len(containers) > 0:
            ...
            __add_label_match(logger, interfaces, label_map, process.labels[label], containers[0].identifier)
...
def __add_process(logger, conf, interfaces, process, chosen, category=None, model=False, label_map=None, peer=None):
    ...
    new.identifier = len(chosen.models) + len(chosen.environment) + 1
    ...
    if label_map:
        for label in label_map["matched labels"].keys():
            for interface in [interfaces.get_or_restore_intf(name) for name
                ...
                new.labels[label].assign_label_interface(interface)
    else:
        for label in new.labels.values():
            for interface in label.interfaces:
                if not label.get_declaration(interface):
                    try:
                        label.assign_label_interface(interfaces.get_or_restore_intf(interface))
                    except KeyError:
...
def __normalize_model(logger, chosen, interfaces):
    ...
    for index1, p1 in enumerate(chosen.environment):
        for index2 in range(index1 + 1, len(chosen.environment)):
            p2 = chosen.environment[index2]
    ...
    for process in [process for process in chosen.models] + \
                   [process for process in chosen.environment]:
...
def __resolve_interface(logger, interfaces, interface, string):
    ...
    logger.debug("Resolve string '{}' as '{}'".format(string, ', '.join([m.identifier for m in matched])))
    return matched
