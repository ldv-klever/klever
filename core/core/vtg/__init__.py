...
class VTG(core.components.Component):
    ...
    def get_strategy(self):
        try:
            self.strategy_name = self.conf['VTG strategy']['name']
            self.strategy = getattr(importlib.import_module('.{0}'.format(self.strategy_name.lower()),
                                                            'core.vtg.strategies'), self.strategy_name.upper())
        except ImportError:
            raise NotImplementedError('Strategy "{0}" is not supported'.format(self.strategy_name))
