...
class RSG(core.avtg.plugins.Plugin):
    ...
    def add_models(self, models):
        ...
        for model in models:
            ...
            model_cc_full_desc_file = {
                'cc full desc file': os.path.relpath(full_desc_file, self.conf['main working directory']),
                'rule spec id': self.conf['rule spec id']
            }
            if 'bug kinds' in self.conf:
                model_cc_full_desc_file['bug kinds'] = self.conf['bug kinds']

            model_grp['cc extra full desc files'].append(model_cc_full_desc_file)
