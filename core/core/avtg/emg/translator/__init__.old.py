...
from core.avtg.emg.translator.instances import _split_into_instances
...
class Translator(metaclass=abc.ABCMeta):
    ...
    def __init__(self, logger, conf, avt, aspect_lines=None):
        ...
        if "terminate approaching to memory limit" in self.conf["translation options"]:
    ...
    def _instanciate_processes(self, analysis, instances, process):
        ...
        if process.name in self.instance_maps[process.category]:
            ...
            cached_map = None
        maps, cached_map = _split_into_instances(analysis, process, self.__resource_new_insts, cached_map)
