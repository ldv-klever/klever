...
# todo: А в чем проблема? У тебя куча примеров перед глазами в другом планировщике.
...
# todo: Нужен, посмотри что он делает для native - он же указывает тебе где запускать контейнеры.
import schedulers.resource_scheduler #hamster ASK: нужен ли именно этот ?
# todo: Без библиотечки ниже ты будешь велосипедостроитльством заниматься. И ведь ты занимаешься там активно ...
import utils

def _execute_cmd():
    # todo: Вместо это функции используй utils или добавь туда функцию лучше, если там чего-то не хваатет. Там есть subprocess.check_output и subprocess.execute. Если нужно добавь туда враппер для subprocess.check_call, а то таскаешь монстров каких-то
    # todo: добавил сюда, чтобы просто красным весь код у меня не светился - убери когда разберешься с этим
    pass
...
class Scheduler(schedulers.SchedulerExchange):
    ...
    def init_scheduler(self):
        ...
        self._init_registry_p_id095()

        self._init_consul()

        self._init_native_sheduler()
    ...
    def _init_k8s_cluster(self, k8s_init_config=None):
        ...
        '''
        # todo: не надо ничего устанавливать при старте планировщика. За это должны отвечать либо а) пользователь б) скрипты установки отдельно от планировщика
        ...
        def k8s_installation(config):
            ...
            # todo: поэтому ничего не надо устанавливать, планировщик нельзя запускать под судо
            # todo: вместо curl нужно использовать питоновские биндинги аля requests, о скорее всего не нужно. Неужели для докер и кубернетс нет биндингов тоже?
            commands = [
            ...
            for line in commands:
                error = _execute_cmd(line, give_output=True)[1]
                if error:
                    return error
            return None
        ...
        if not k8s_init_config:
            temp_config = self._get_k8s_init_config()
        else:
        ...
        def make_master_conf_file(conf):
            ...
            :return: File (path).
            """
            # todo: не очень понимаю зачем файл генерить а не взять готовый от пользователя или репозитория. Ну ладно, пусть пока так.
        ...
        # todo: Настройку докер и кубернетс должен делать либо скрипт либо пользователь по инструкции снаружи. Этот код никогда не будет выполнятся под судо, не будет и не должен ничего устанавливать и поднимать сервисы - он пользователь.
        # todo: более того, на клиентах планировщик никогда не запускается - он работает на мастере. На клиентах запускается только клиент - scheduler/bin/client-controller
        if "master configuration" in temp_config:
            ...
            _execute_cmd(cmd_string)#hamster TODO: add check error from command
        else:
            _execute_cmd("sudo kubeadm init")#hamster TODO: add check error from command
    ...
    def _init_registry_p_id095(self):
        ...
        :return:
        """
        # todo: тоже самое - этот код либо в отдельный скрипт, либо выполнять пользователю самостоятельно
        commands = [
            "docker run -d \
            -p 5001:5000 \
            --restart=always \
        ...
        for cmd in commands:
            _execute_cmd(cmd, timeout=5, give_output=True)
...
# Some utility methods:
    ...
    #hamster TODO: method to execute chosen commands in shell - огромный велосипед. Если тебе нужно получить output используй utils.get_output. Выше вообщем смотри мой комментарий.
    ...
    #         signal.signal(signal.SIGINT, handler)
        ...
        #
        #     if give_output:
        #         return tuple(return_output, return_error)
        #     else:
    ...
    def _check_k8s_component_version(component_k8s):
        ...
        if not component_k8s:
            ...
            temp_return = _execute_cmd("kubeadm version", timeout=5, give_output=True)
            ...
            temp_return = _execute_cmd("kubectl version", timeout=5, give_output=True)
            for line in temp_return[0]:
                find_version = re.search(r'GitVersion:"v.*"', line)
                if find_version:
...
# todo: Зачем она тебе здесь? Тебе и нужно использовать функции из utils или доделать там то, чего реально не хватает, а не писать велосипеды.
