...
def configure_controller_and_schedulers(logger, development, deploy_dir, prev_deploy_info):
    ...
    scheduler_conf = os.path.join('klever', 'klever', 'scheduler', 'conf')

    with Cd(deploy_dir):
        logger.info('Configure Klever Controller')
        with open(os.path.join(scheduler_conf, 'controller.json')) as fp:
        ...
        with open(os.path.join(scheduler_conf, 'native-scheduler.json')) as fp:
        ...
        with open(os.path.join(scheduler_conf, 'job-client.json')) as fp:
        ...
        if need_verifiercloud_scheduler(prev_deploy_info):
            ...
            with open(os.path.join(scheduler_conf, 'verifiercloud-scheduler.json')) as fp:
