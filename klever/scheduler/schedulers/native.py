...
from klever.scheduler.schedulers.runners import Runner


class BaseNativeRunner(Runner):
    # Common methods of Native and Docker runners
    ...
    def init(self):
        ...
        with open(self.conf["scheduler"]["job client configuration"], encoding="utf-8") as fh:
            self._job_conf_prototype = yaml.safe_load(fh.read())
    ...
    def _process_task_result(self, identifier, result, description):
        ...
        """
        return self._check_solution(identifier, result, mode='task')

    def _process_job_result(self, identifier, result):
        """
        Process job status and send results to the server.
        ...
        :raise SchedulerException: in case of ERROR status.
        """
        return self._check_solution(identifier, result, mode='job')
    ...
    def _prepare_solution(self, identifier, configuration, mode='task'):
        ...
        if mode == 'task':
            ...
            client_conf["common"]["working directory"] = work_dir
            client_conf["Klever Core conf"] = klever_core_conf
        ...
        return multiprocessing.Process(None, self._process_starter, identifier, [mode, client_conf])
    ...
    def _cancel_solution(self, process, identifier, mode='task'):
        ...
        return self._postprocess_solution(identifier, process.exitcode, mode, True)

    def _check_solution(self, identifier, result, mode='task'):
        """
        ...
        return self._postprocess_solution(identifier, result, mode)
...
class TryLessMemoryRunner(BaseNativeRunner):
...
class Native(TryLessMemoryRunner):
    """
    ...
    def _is_done(self, item):
        done = item.get("future") and item["future"].done()
        result = item["future"].result() if done else None
        return done, result

    def _is_cancelable(self, identifier, item, mode='task'):
        if item.get("future") and not item["future"].cancel():
            if mode == 'task':
                return self._task_processes.get(identifier, None)
            return self._job_processes.get(identifier, None)
        return None

    def _prepare_solution(self, identifier, configuration, mode='task'):
        process = super()._prepare_solution(identifier, configuration, mode)
        if mode == 'task':
    ...
    def _postprocess_solution(self, identifier, result, mode, fault_tolerant=False):
        ...
        return super()._postprocess_solution(identifier, result, mode, fault_tolerant)
