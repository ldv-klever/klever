...
def _compare_process(one, two):
    ...
    _compare_process_sequences(one['process'], two['process'])
...
def _compare_process_sequences(one, two):
    p1 = Process('test1')
    p2 = Process('test2')
    parse_process(p1, one)
    parse_process(p2, two)

    assert p1.actions.initial_action == p2.actions.initial_action
    assert len(p1.actions) == len(p2.actions)
    assert sorted(map(str, p1.actions.final_actions)) == sorted(map(str, p2.actions.final_actions))
