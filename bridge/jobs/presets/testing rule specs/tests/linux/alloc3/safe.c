...
static void memory_allocation_2(void)
{
	...
	// ALLOC
	struct page *mem_1 = alloc_pages(GFP_NOIO, order);
	struct page *mem_2 = alloc_pages_vma(GFP_NOIO, order, vma, addr, node);
	struct my_struct *mem_3 = kmalloc(sizeof(mem_3), GFP_NOIO);
	struct sk_buff *mem_4 = alloc_skb(sizeof(mem_3), GFP_NOIO);
	struct sk_buff *mem_5 = alloc_skb_fclone(sizeof(mem_3), GFP_NOIO);
	struct sk_buff *mem_6 = skb_copy(mem_5, GFP_NOIO);
	struct sk_buff *mem_7 = skb_share_check(mem_6, GFP_NOIO);
	struct sk_buff *mem_8 = skb_clone(mem_7, GFP_NOIO);
	struct sk_buff *mem_9 = skb_unshare(mem_8, GFP_NOIO);
	struct sk_buff *mem_10 = __netdev_alloc_skb(dev, length, GFP_NOIO);
	struct sk_buff *mem_11 = skb_copy_expand(mem_10, newheadroom, newtailroom, GFP_NOIO);
	struct my_struct *mem_12 = usb_alloc_coherent(dev_usb, sizeof(mem_3), GFP_NOIO, dma);
	struct urb *mem_13 =  usb_alloc_urb(iso_packets, GFP_NOIO);
	struct my_struct *mem_14 = kmalloc_node(sizeof(mem_3), GFP_NOIO, node);
	struct my_struct *mem_15 = kmem_cache_alloc(cache, GFP_NOIO);
	struct my_struct *mem_16 = mempool_alloc(pool, GFP_NOIO);
	struct my_struct *mem_17 = dma_pool_alloc(pool1, GFP_NOIO, dma);
	struct my_struct *mem_18 = kcalloc(length, sizeof(mem_3), GFP_NOIO);
	struct my_struct *mem_19 = krealloc(mem_18, sizeof(mem_3), GFP_NOIO);
	struct my_struct *mem_20 = dma_zalloc_coherent(device, sizeof(mem_3), dma, GFP_NOIO);
	struct my_struct *mem_21 = dma_alloc_coherent(device , sizeof(mem_3), dma, GFP_NOIO);

	// ALLOC with int
	int x_1 = __get_free_pages(GFP_NOIO, sizeof(mem_3));
	int x_2 = usb_submit_urb(mem_13, GFP_NOIO);
	int x_3 = mempool_resize(pool, order, GFP_NOIO);
	int x_4 = pskb_expand_head(mem_8, order, node, GFP_NOIO);

	// zalloc
	struct my_struct *mem_z1 = kzalloc(sizeof(mem_3), GFP_NOIO);
	struct my_struct *mem_z2 = kmem_cache_zalloc(cache, GFP_NOIO);
	struct my_struct *mem_z3 = kzalloc_node(sizeof(mem_3), GFP_NOIO, node);

	// macro
	struct page *mem_m1 = alloc_pages(GFP_NOIO, order);
	struct page *mem_m2 = alloc_page_vma(GFP_NOIO, vma, addr);
}
