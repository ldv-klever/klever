...
STATE USEALL Init :
  ...
  MATCH RETURN {$1=ldv_sysfs_create_group($?)} -> ASSUME {((int)$1) == 0} ENCODE {sysfs_state=sysfs_state+1;} GOTO Inc;
...
STATE USEALL Inc :
  ...
  MATCH RETURN {$1=ldv_sysfs_create_group($?)} -> ASSUME {((int)$1) >  0} GOTO Stop;
  MATCH CALL {ldv_sysfs_remove_group($?)} -> ASSUME {sysfs_state >  1} ENCODE {sysfs_state=sysfs_state-1;} GOTO Inc;
  MATCH CALL {ldv_sysfs_remove_group($?)} -> ASSUME {sysfs_state <= 1} ENCODE {sysfs_state=sysfs_state-1;} GOTO Init;
