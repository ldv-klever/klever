...
STATE USEALL G0_C0_H0 :
  ...
  MATCH CALL {ldv_unregister_class($?)} -> ERROR("linux:class::double deregistration");
  ...
  MATCH CALL {ldv_destroy_class($1)} -> ASSUME {((unsigned long)$1) <= 2012; ((unsigned long)$1) > 0} ERROR("linux:class::double deregistration");
