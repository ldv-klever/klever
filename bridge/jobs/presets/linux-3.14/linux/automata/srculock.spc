...
STATE USEFIRST Init :
  ...
  MATCH CALL {ldv_srcu_read_lock($?)} -> ENCODE {srcu_state=srcu_state+1;} GOTO Inc;
  MATCH CALL {ldv_srcu_read_unlock($?)} -> ERROR("linux:srculock::more unlocks");
  
STATE USEALL Inc :
  MATCH CALL {ldv_srcu_read_lock($?)} -> ENCODE {srcu_state=srcu_state+1;} GOTO Inc;
  MATCH CALL {ldv_srcu_read_unlock($?)} -> ASSUME {srcu_state != 1;} ENCODE {srcu_state=srcu_state-1;} GOTO Inc;
  MATCH CALL {ldv_srcu_read_unlock($?)} -> ASSUME {srcu_state == 1;} ENCODE {srcu_state=srcu_state-1;} GOTO Init;
