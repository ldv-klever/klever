...
void ldv_rcu_read_unlock(void)
{
	...
	ldv_assert("linux:kernel:rcu:update:lock::more unlocks", ldv_rcu_nested > 0);
...
{
	/* ASSERT checks the count of opened rcu_lock sections */
	ldv_assert("linux:kernel:rcu:update:lock::locked at read section", ldv_rcu_nested == 0);
}
...
{
	/* ASSERT checks the count of opened rcu_lock sections */
	ldv_assert("linux:kernel:rcu:update:lock::locked at exit", ldv_rcu_nested == 0);
}
